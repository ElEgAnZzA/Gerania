public class Spell {
    ...
    public void cast(int casterIndex, Point click, MainGame mainGame){
        ...
        Point realClick = new Point(click.getX(), MainGame.SCREEN_HEIGHT -click.getY()); //Точка - положение точки нажатия в системе координат игры
        Point casterPoint = new Point(casterCenterX, casterCenterY); //Точка - положение центра существа, использовавшего заклинание в системе координат игры
        Point targetPoint;
        //Направление для снаряда (если таковой есть)
        Vector projectileVel = new Vector(new Point(casterCenterX, casterCenterY), new Point(mainGame.cameraX+realClick.getX(), mainGame.cameraY+realClick.getY()));

        switch (type){
            case (-4)://Ультразвук летучей мыши
                mainGame.creatures[mainGame.kCreatures] = new Creature(casterCenterX, casterCenterY, 16, 16, 1, mainGame.endTime, mainGame.kCreatures);
            ...
            case (2): //Лечение (на 15 хп)
                mainGame.creatures[casterIndex].setHealth(mainGame.creatures[casterIndex].getHealth()+45);
                break;
            ...
            case (4): //Самонаводящийся снаряд
                if(mainGame.kCreatures>1) {
                    ...
                    //Перебираем все существа, находим ближайшее:
                    for (int i = 0; i < mainGame.kCreatures; i++) {
